 ...
 */
if (!function_exists('swift_staff_dashboard_cb')) {

    function swift_staff_dashboard_cb() {

        include_once( ABSPATH . WPINC . '/feed.php' );
        wp_enqueue_script('swift-form-jstz', SWIFTSTAFF_PLUGIN_URL . 'admin/js/jstz.min.js', '', '', true);
        wp_enqueue_style('swift-dashboard', SWIFTSTAFF_PLUGIN_URL . 'admin/css/swift-dashboard.css', '', '', '');
        wp_enqueue_script(SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard-script', SWIFTSTAFF_PLUGIN_URL . 'admin/js/swift-dashboard.js', array('jquery'), '', true);
        wp_enqueue_style('swiftcloud-fontawesome', SWIFTSTAFF_PLUGIN_URL . 'css/font-awesome.min.css', '', '', '');

        $subscribed = get_option(SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_subscribe');
        $subs_checkbox_flag = isset($_COOKIE[SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_unsubscribe']) && $_COOKIE[SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_unsubscribe'] == 1 ? "block" : "none";
        ...
        <div class="wrap dashboard-wrap">
            ...
            <?php } ?>
            <!--subscribe box-->
            <div class="dashboard-subscribe-block" id="swift_staff_subscribe" style="display: <?php echo $subs_box_flag; ?>;">
                ...
                <form method="post" id="frm_staff_dashboard_subscribe">
                    ...
                    <input type="hidden" name="formid" value="652" id="formid" />
                    ...
                    <div class="dashboard-col-8 col-left">
                        <div class="col-dashboard-block">
                            ...
                            <div class="col-dashboard-block-content">
                                <?php
                                $rss = fetch_feed('https://SwiftCRM.com/support/tag/staff/feed');
                                $maxitems = 0;
                                ...
                                <?php if ($maxitems == 0) : ?>
                                    <div class="col-dashboard-item">
                                        <p><?php _e('No items', 'swift_helpdesk'); ?></p>
                                    </div>
                                <?php else : ?>
 ...
 *      cookie value : 0 || 1
 */
add_action('wp_ajax_' . SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox', SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox_callback');
add_action('wp_ajax_nopriv_' . SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox', SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox_callback');
if (!function_exists(SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox_callback')) {

    function swift_staff_dashboard_subscribe_checkbox_callback() {
        if (isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox') {
            setcookie(SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_unsubscribe', "", time() - 1, "/", '');
            echo "1";
        }
        wp_die();
    }
...
add_action('wp_ajax_' . SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_close_subscribe', SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_close_subscribe_callback');
add_action('wp_ajax_nopriv_' . SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_close_subscribe', SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_close_subscribe_callback');
if (!function_exists(SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_close_subscribe_callback')) {

    function swift_staff_dashboard_close_subscribe_callback() {
        if (isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_close_subscribe') {
            setcookie(SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_unsubscribe', "1", time() + (10 * 365 * 24 * 60 * 60), "/", '');
            echo "1";
        }
        wp_die();
    }
...
add_action('wp_ajax_' . SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_subscribe', SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_subscribe_callback');
add_action('wp_ajax_nopriv_' . SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_subscribe', SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_subscribe_callback');

if (!function_exists(SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_subscribe_callback')) {

    function swift_staff_dashboard_subscribe_callback() {
        check_ajax_referer('swiftdashboard_subs_form', 'swiftdashboard_subs_form');
        if (isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_subscribe') {
            parse_str($_POST['data'], $subscribe_form_data);

            update_option(SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_subscribe', true);
            setcookie(SWIFTSTAFF_PLUGIN_PREFIX . 'dashboard_unsubscribe', "", time() - 1, "/", '');
